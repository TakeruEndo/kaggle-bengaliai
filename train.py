# use
# https://www.kaggle.com/corochann/bengaliaicv19feather
...
from src.GridMask import GridMask
...
data0 = pd.read_feather('input/train_image_data_0.feather')
data1 = pd.read_feather('input/train_image_data_1.feather')
data2 = pd.read_feather('input/train_image_data_2.feather')
data3 = pd.read_feather('input/train_image_data_3.feather')

data_full = pd.concat([data0, data1, data2, data3], ignore_index=True)


class GraphemeDataset(Dataset):
    def __init__(self, df, label=None, type='train', transforms=None):
        self.df = df
        if type == 'train':
            self.label = label
    ...
    def __getitem__(self, idx):
        ...
        if type == 'train':
            return augmented_image, label1, label2, label3
        else:
...
for epoch in range(epochs):
    ...
    train_image = GraphemeDataset(reduced_data, reduced_train, transforms=transforms_train)
...
torch.save(model.state_dict(), 'resnet18_50epochs_saved_weights.pth')
